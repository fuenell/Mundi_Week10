	...
	for (int32 i = 0; i < PolygonCount; ++i)
	{
		...
		if (PolygonSize == 3)
		{
			...
			for (int32 j = 0; j < 3; ++j)
			{
				...
				TriVertices[j] = FVector(
					static_cast<float>(Position[0]),
					static_cast<float>(Position[1]),
					static_cast<float>(Position[2])
				);

				// Normal 가져오기 (와인딩 순서를 반전했으므로 노멀도 반전)
				FbxVector4 FbxNormal;
				Mesh->GetPolygonVertexNormal(i, j, FbxNormal);
				TriNormals[j] = FVector(
				...
				if (UVElement)
				{
					...
					if (UVIndex >= 0 && UVIndex < UVElement->GetDirectArray().GetCount())
					{
						...
						TriUVs[j].X = static_cast<float>(FbxUV[0]);
						TriUVs[j].Y = 1.0f - static_cast<float>(FbxUV[1]); // DirectX UV 좌표계 (Y 반전)
					}
				}
			}

			// DirectX 와인딩 순서로 역순 추가 (2, 1, 0)
			for (int32 j = 2; j >= 0; --j)
			{
				OutMeshData->Vertices.Add(TriVertices[j]);
				OutMeshData->Normal.Add(TriNormals[j]);
				OutMeshData->UV.Add(TriUVs[j]);

				// 인덱스 추가
				OutMeshData->Indices.Add(VertexOffset + static_cast<uint32>(OutMeshData->Vertices.Num()) - 1);
			}
		}
	}
}
...
void UFBXImporter::ProcessSkeletalMesh(FbxMesh* Mesh, FSkeletalMeshData* OutSkeletalMeshData)
{
	...
	for (int32 i = 0; i < PolygonCount; ++i)
	{
		...
		if (PolygonSize == 3)
		{
			...
			for (int32 j = 0; j < 3; ++j)
			{
				...
				if (UVElement)
				{
					...
					if (UVIndex >= 0 && UVIndex < UVElement->GetDirectArray().GetCount())
					{
						FbxVector2 FbxUV = UVElement->GetDirectArray().GetAt(UVIndex);
						TriUVs[j].X = static_cast<float>(FbxUV[0]);
						TriUVs[j].Y = 1.0f - static_cast<float>(FbxUV[1]);
					}
				}
			}
			...
			for (int32 j = 2; j >= 0; --j)
			{
				TempVertices.Add(TriVertices[j]);
				TempNormals.Add(TriNormals[j]);
				TempUVs.Add(TriUVs[j]);

				// 인덱스 추가
				OutSkeletalMeshData->Indices.Add(VertexOffset + static_cast<uint32>(TempVertices.Num()) - 1);
			}
		}
	}

	// 최종 데이터에 추가
	OutSkeletalMeshData->Vertices.Append(TempVertices);
	OutSkeletalMeshData->Normal.Append(TempNormals);
	OutSkeletalMeshData->UV.Append(TempUVs);

	// 스킨 웨이트 추출
	ExtractSkinWeights(Mesh, OutSkeletalMeshData->Skeleton, OutSkeletalMeshData->BoneWeights, static_cast<int32>(TempVertices.Num()));
}
...
void UFBXImporter::ExtractSkinWeights(FbxMesh* Mesh, const FSkeleton& Skeleton, TArray<FBoneWeight>& OutBoneWeights, int32 VertexCount)
{
	...
	// 본 웨이트 배열 초기화
	OutBoneWeights.SetNum(VertexCount);
